...
public final class FileUtils {
     ...
     * @return {@code true}: 删除成功<br>{@code false}: 删除失败
     */
    public static boolean deleteFilterFileInDir(final File dir, final String pathSuffix) {
        return deleteFilesInDirWithFilter(dir, new FileFilter() {
            @Override
            public boolean accept(File pathname) {
                if (pathname.isFile() && pathname.getName().contains(pathSuffix)){
                    return true;
                }
                return false;
            }
