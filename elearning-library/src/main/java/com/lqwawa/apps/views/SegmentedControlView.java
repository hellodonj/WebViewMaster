...
public class SegmentedControlView extends RadioGroup {
    ...
    //Item organization
    public LinkedHashMap<String, String> itemMap = new LinkedHashMap<String, String>();
    ...
    private void update() {
        ...
        int twoDP = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1, getResources
                ().getDisplayMetrics());
        ...
        while (itemIterator.hasNext()) {
            ...
            if (itemMap.size() == 1){
                GradientDrawable gradientDrawable = (GradientDrawable) mCtx.getResources().getDrawable(R
                        .drawable.one_option).mutate();
                gradientDrawable.setStroke(twoDP, selectedColor);
                gradientDrawable.setColor(unselectedColor);
                GradientDrawable gradientSelect = (GradientDrawable) mCtx.getResources()
                        .getDrawable(R.drawable.one_option_selected).mutate();
                gradientSelect.setColor(selectedColor);
                gradientSelect.setStroke(twoDP, selectedColor);

                StateListDrawable oneStateListDrawable = new StateListDrawable();
                oneStateListDrawable.addState(new int[]{-android.R.attr.state_checked}, gradientSelect);
                oneStateListDrawable.addState(new int[]{android.R.attr.state_checked}, gradientSelect);
                if (mSdk < Build.VERSION_CODES.JELLY_BEAN) {
                    rb.setBackgroundDrawable(oneStateListDrawable);
                } else {
                    rb.setBackground(oneStateListDrawable);
                }
            } else {
                if (i == 0) {
                    //Left
...
//            rb.setTypeface(null, Typeface.BOLD);
        ...
        for (RadioButton option : options) {
            if (equalWidth) {
                option.setWidth((int) (textWidth + (twoDP * 40)));
            }
            this.addView(option);
        }
