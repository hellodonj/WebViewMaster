...
public class ExpandableTextView extends TextView {
    ...
    private boolean mArrowType = false;
    ...
    private boolean needOnClickExpand = true;
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            } else if (attr == R.styleable.ExpandableTextView_etv_ArrowType) {
                mArrowType = a.getBoolean(attr, false);
            }
        }
        a.recycle();
    }
    ...
    private void init() {
        ...
        if (TextUtils.isEmpty(mToExpandHint)) {
            ...
            mToExpandHint = getResources().getString(R.string.to_expand_hint_prefix);
        }
        if (TextUtils.isEmpty(mToShrinkHint)) {
            mToShrinkHint = getResources().getString(R.string.to_shrink_hint);
            mToShrinkHint = getResources().getString(R.string.to_shrink_hint_prefix);
        }
        if (mToggleEnable) {
    ...
    private CharSequence getNewTextByConfig() {
        ...
        switch (mCurrState) {
            ...
            case STATE_EXPAND: {
                ...
                if (mArrowType) {
                    Drawable drawable = getResources().getDrawable(R.drawable.arrow_up_gray_ico);
                    drawable.setBounds(0, 0, drawable.getIntrinsicWidth(),
                            drawable.getIntrinsicHeight());
                    ImageSpan span = new ImageSpan(drawable, ImageSpan.ALIGN_BASELINE);
                    ssbExpand.setSpan(span,
                            ssbExpand.length() - getLengthOfString(mToExpandHint),
                            ssbExpand.length(),
                            Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                } else {
    ...
    public void setNeedOnClickExpand(boolean needOnClickExpand) {
        this.needOnClickExpand = needOnClickExpand;
    }
