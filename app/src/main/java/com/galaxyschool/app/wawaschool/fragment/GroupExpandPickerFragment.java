...
//                    holder.selectorView.invalidate();
    ...
    private void loadGroups() {
        ...
        params.put("IsCheckCourseOnlineStatus",true);
    ...
    private void updateViews(ContactsSchoolListResult result) {
        ...
        for (ContactsSchoolInfo school : list) {
            if (this.groupType == ContactsPickerActivity.GROUP_TYPE_SCHOOL) {
                if (this.roleType == ContactsPickerActivity.ROLE_TYPE_TEACHER) {
                    if (!school.isTeacher()) {
                        continue;
                    }
                } else if (this.roleType == ContactsPickerActivity.ROLE_TYPE_STUDENT) {
                    if (!school.isStudent()) {
                        continue;
                    }
                } else if (this.roleType == ContactsPickerActivity.ROLE_TYPE_PARENT) {
                    if (!school.isParent()) {
                        continue;
                    }
                }
                school.setClassMailList(null);
                if (isOnlineClass){
                    if (school.isOnlineSchool()){
                        groupList.add(school);
                    }
                } else {
                    if (school.isOnlineSchool() && (this.pickerType & ContactsPickerActivity.PICKER_TYPE_GROUP) != 0){
            ...
            Iterator<ContactsClassInfo> it = school.getClassMailList().iterator();
            while (it.hasNext()) {
                ContactsClassInfo obj = it.next();
                if (obj.isHistory()) {
                    it.remove();
                    continue;
                }

                if (obj.getNewCourseOnlineStatus() == 1){
                    //移除授课计划结束的班级
                    it.remove();
                    continue;
                }
                ...
                if (tempData) {
                    if (obj.getNewCourseOnlineStatus() == 2) {
                        //移除历史授课
                        it.remove();
                        continue;
                    }
                ...
                if (this.groupType == ContactsPickerActivity.GROUP_TYPE_CLASS) {
                    ...
                    if (getArguments().getBoolean(ContactsPickerActivity.EXTRA_PUBLISH_CHAT_RESOURCE)
                            && obj.isChatForbidden()) {
                        it.remove();
                        continue;
                    }
            ...
            if (school.getClassMailList().size() > 0) {
                if (isOnlineClass){
                    ...
                    if (uploadParameter != null
                            && uploadParameter.getShareType() == ShareType.SHARE_TYPE_COMMENT
                            && school.isOnlineSchool()){
...
//            notifyPickerBar(hasSelectedItems());
    ...
    private void notifyPickerBar(boolean selected) {
        ...
        if (multiSelection) {
            PickerClassAndGroupFragment groupFragment = (PickerClassAndGroupFragment) GroupExpandPickerFragment.this.getParentFragment();
            if (groupFragment != null){
                groupFragment.setItemSelected(1,selected);
            }
        }
    }
...
//        ft.add(R.id.contacts_layout, fragment, GroupMemberPickerFragment.TAG);
        ...
        ft.hide(GroupExpandPickerFragment.this);
