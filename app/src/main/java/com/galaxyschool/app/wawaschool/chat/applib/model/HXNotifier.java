...
public class HXNotifier {
    ...
    public synchronized void onNewMesg(List<EMMessage> messages) {
        if(messages.get(messages.size()-1).getBooleanAttribute("em_ignore_notification", false)){
            return;
        }
        // 判断app是否在后台
        if (!EasyUtils.isAppRunningForeground(appContext)) {
...
//        String username = message.getFrom();
        ...
        try {
            ...
            //这里使用Vibrator和SoundPool来产生声音和震动，不再使用AndroidNotification系统默认的声音和震动
            //- 为AndroidNotification设置自定义的声音和震动，因为推出app声音会响两次
//            PrefsManager prefsManager = ((MyApplication)appContext).getPrefsManager();
