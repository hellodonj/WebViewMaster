...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    private boolean isTabTextAllCaps = false;//默认不是全部字母大写
    ...
    //是否要定制化tab的颜色
    private boolean isChangeTabTextColor;
    //定制化tab的下标值
    private List<Integer> changeTabTextColorIndex;
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            if (isCustomTabs) {
                ...
                if (isChangeTabTextColor){
                    tabView = LayoutInflater.from(getContext()).inflate(R.layout.psts_tab_red_tag,
                            this, false);
                } else {
    ...
    private void addTab(final int position, CharSequence title, View tabView) {
        ...
        if (isChangeTabTextColor) {
            //根据条件判断是否显示小红点
            ImageView redPointView = (ImageView) tabView.findViewById(R.id.red_point);
            if (redPointView != null) {
                if (displayTabAppointTabColor(position)) {
    ...
    private boolean displayTabAppointTabColor(int position){
        if (isChangeTabTextColor && changeTabTextColorIndex != null && changeTabTextColorIndex
                .size() > 0 && changeTabTextColorIndex.contains(position)){
            return true;
        }
        return false;
    }

    public void setChangeTabTextColor(boolean isChangeTabTextColor){
        this.isChangeTabTextColor = isChangeTabTextColor;
    }

    public void setChangeTabTextColorIndex(List<Integer> tabTextColorIndex){
        this.changeTabTextColorIndex = tabTextColorIndex;
    }
}
