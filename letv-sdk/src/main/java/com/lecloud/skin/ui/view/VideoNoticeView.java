...
public class VideoNoticeView extends RelativeLayout implements View.OnTouchListener{
    ...
    private void initView() {
        ...
        mEndOnline= (Button) findViewById(R.id.btn_end_online);
        if (mEndOnline!=null){
            mEndOnline.setOnTouchListener(this);
        }
...
//        mErrorReport.setText(mContext.getString(R.string.le_back));
        int status=-1;
        if (!TextUtils.isEmpty(cancel)){
            status=Integer.valueOf(cancel);
        }
        mRePlayListener.showViewText(this,status);
...
//            mEventCode = data.getString(PlayerParams.KEY_RESULT_ERROR_CODE, "");
            if (status == StatusCode.MEDIADATA_NETWORK_ERROR) {
                ...
                videoTips(mEventCode, errMsg, getText(R.string.letv_notice_message), getText(R.string.reconnect),"");
            ...
            videoTips("" + 10000, getText(R.string.net_fail), getText(R.string.net_error), getText(R.string.reconnect),"");
        }
    }
    ...
    public void processActionStatus(int status) {
        ...
        videoTips("", "", errMsg, getText(R.string.reconnect),String.valueOf(status));
    }
...
//                mEventCode = "" + event;
                if(code == StatusCode.PLAY_ERROR_NONETWORK){
                	...
                	videoTips("" + code, getText(R.string.play_fail), getText(R.string.letv_notice_message), getText(R.string.reconnect),"");
                }
                break;
...
//                }
                if (isLive) {
                    mEventCode = "" + 10000;
                    videoTips("" + 10000, getText(R.string.play_fail), getText(R.string.letv_notice_message), getText(R.string.reconnect),"");
                }
                break;
            default:
                break;
    ...
    public interface IReplayListener {
        ...
        void onRePlay();
        void continuePlay() throws UnsupportedEncodingException;
        void showViewText(VideoNoticeView view, int status);
        void endAirclassOnline();
    }
...
//                return true;
                try {
                    mRePlayListener.continuePlay();
                }catch (Exception exception){
                }

			}else if (v.getId()==R.id.btn_end_online){
                    mRePlayListener.endAirclassOnline();
            }
		}
		return true;
	}
