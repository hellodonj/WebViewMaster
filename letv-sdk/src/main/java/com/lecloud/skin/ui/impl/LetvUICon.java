...
//	public LetvUIListener letvUIListener;
    ...
    private boolean isPingBan=false;
    private boolean isSmall=true;
    private boolean isNeedTopTitle=false;
    static class MyHandler extends Handler {
        WeakReference<LetvUICon> mReference;

        MyHandler(LetvUICon uiCon) {
            mReference = new WeakReference<LetvUICon>(uiCon);
        }
            ...
            switch (msg.what) {
                ...
                case OrientationSensorUtils.ORIENTATION_1:// 正竖屏
                    if (uiCon.mLetvUIListener != null) {
                        ...
                        if (bundle!=null){
                            boolean tempData=bundle.getBoolean("tempData");
                            uiCon.mLetvUIListener.onBackPress(tempData);
                        }
...
//    };
    ...
    public void setRequestedOrientation(int requestedOrientation, View view) {
        ...
        if (requestedOrientation == ILetvVodUICon.SCREEN_LANDSCAPE) {
            ...
            if(!VodVideoSettingUtil.getInstance().isNewUi()){
...
//		canGesture(canGesture);
    ...
    public void show() {
        ...
        if (mLandScape) {
            canGesture(canGesture);
            mLargeMediaController.setVisibility(VISIBLE);
            mSmallMediaController.setVisibility(INVISIBLE);
        } else {
            mSmallMediaController.setVisibility(VISIBLE);
            mLargeMediaController.setVisibility(INVISIBLE);
        }
        //如果是平板
        if (isPingBan){
            if (isSmall){
...
//        mSeekable = seekable;
    ...
    public void setIsFromPingBan(boolean isPingBan,boolean isSmall){
        this.isPingBan=isPingBan;
        this.isSmall=isSmall;
    }
    public void setIsNeedTopTitle(boolean isNeedTopTitle){
