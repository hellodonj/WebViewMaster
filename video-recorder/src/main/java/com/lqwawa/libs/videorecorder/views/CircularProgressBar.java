package com.lqwawa.libs.videorecorder.views;
...
import com.lqwawa.libs.videorecorder.R;
...
public class CircularProgressBar extends View {
    ...
    private float canvasRotation = 0.0f;
    ...
    public void setCanvasRotation(float rotation) {
        if (canvasRotation != rotation) {
    ...
    private void setupPaints() {
        ...
        if (contourShader != null) {
            contourPaint.setShader(contourShader);
        } else {
    ...
    private void setupBounds() {
        ...
        circleBounds.top *= circleScale;
        circleBounds.left *= circleScale;
        circleBounds.bottom *= circleScale;
        circleBounds.right *= circleScale;
    ...
    private void parseAttributes(TypedArray a) {
        barWidth = (int) a.getDimension(R.styleable.CircularProgressBar_pwBarWidth, barWidth);
        rimWidth = (int) a.getDimension(R.styleable.CircularProgressBar_pwRimWidth, rimWidth);
        spinSpeed = (int) a.getDimension(R.styleable.CircularProgressBar_pwSpinSpeed, spinSpeed);
        barLength = (int) a.getDimension(R.styleable.CircularProgressBar_pwBarLength, barLength);

        delayMillis = a.getInteger(R.styleable.CircularProgressBar_pwDelayMillis, delayMillis);
        if (delayMillis < 0) { delayMillis = 10; }

        // Only set the text if it is explicitly defined
        if (a.hasValue(R.styleable.CircularProgressBar_pwText)) {
            setText(a.getString(R.styleable.CircularProgressBar_pwText));
        }

        barColor = a.getColor(R.styleable.CircularProgressBar_pwBarColor, barColor);
        textColor = a.getColor(R.styleable.CircularProgressBar_pwTextColor, textColor);
        rimColor = a.getColor(R.styleable.CircularProgressBar_pwRimColor, rimColor);
        circleColor = a.getColor(R.styleable.CircularProgressBar_pwCircleColor, circleColor);
        contourColor = a.getColor(R.styleable.CircularProgressBar_pwContourColor, contourColor);

        textSize = (int) a.getDimension(R.styleable.CircularProgressBar_pwTextSize, textSize);
        contourSize = a.getDimension(R.styleable.CircularProgressBar_pwContourSize, contourSize);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.rotate(canvasRotation, layoutWidth / 2, layoutHeight / 2);
    ...
    */
    ...
    public void incrementProgressByPercent(double percent) {
        int amount = (int) (360 * percent);
...
//    	}
    ...
    public Shader getContourshader() {
        return contourPaint.getShader();
    }

    public void setContourShader(Shader shader) {
//        this.contourPaint.setShader(shader);
        this.contourShader = shader;
    }
