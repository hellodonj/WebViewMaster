...
public class UserHelper {
     ...
     * @param callback
     */
    public static void requestTutorStarLevel(@NonNull String tutorMemberId,
                                             @NonNull DataSource.Callback<TutorStarLevelEntity> callback) {
        // 准备数据
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("tutorMemberId", tutorMemberId);

        RequestParams params = new RequestParams(AppConfig.ServerUrl.PostGetTutorStarLevel);
        params.setAsJsonContent(true);
        params.setBodyContent(jsonObject.toJSONString());
        params.setConnectTimeout(10000);
        ...
        x.http().post(params, new StringCallback<String>() {
                    ...
                    public void onSuccess(String str) {
                        LogUtil.i(LiveHelper.class, "request " + params.getUri() + " result :" + str);
                        TypeReference<ResponseVo<TutorStarLevelEntity>> typeReference =
                                new TypeReference<ResponseVo<TutorStarLevelEntity>>() {
                                };
                        ResponseVo<TutorStarLevelEntity> responseVo = JSON.parseObject(str, typeReference);
                        if (responseVo.isSucceed()) {
                            if (EmptyUtil.isNotEmpty(callback)) {
                                callback.onDataLoaded(responseVo.getData());
                            }
                        } else {
                            if (EmptyUtil.isNotEmpty(responseVo)) {
                                Factory.decodeRspCode(responseVo.getCode(), callback);
                            }
                        }
                    }
