...
public class FaceInputFilterEditText extends EditText {
    ...
    public FaceInputFilterEditText(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mContext = context;
        initEditText();
    }

    public FaceInputFilterEditText(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initEditText() {
        addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (!resetText) {
                    if (before != 0) {
                        //这里是关键代码，因为输入的时候，这些回调会被调用2次，第一次before==0,而第二次的时候，before==输入字符串的长度，当手机是google输入法等手机系统自带的输入法时，应用就会crash,报系统错误：java.lang.IndexOutOfBoundsException: getChars (2 ... 6) ends beyond length 4，加上这个非0判断后，就会解决此问题。
                        return;
                    }
