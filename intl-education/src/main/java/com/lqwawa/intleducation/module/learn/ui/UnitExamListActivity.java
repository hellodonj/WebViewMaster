...
import com.lqwawa.intleducation.module.learn.adapter.UnitExamListAdapter;
 ...
 * 单元测试列表
 */
public class UnitExamListActivity extends MyBaseActivity implements View.OnClickListener,
        ListView.OnItemClickListener {

    private UnitExamListAdapter testListAdapter;
    ...
    public static void start(Activity activity, String courseId, String chapterId, boolean canEdit,
                             String memberId, String schoolId, boolean isFromMyCourse) {
        activity.startActivity(new Intent(activity, UnitExamListActivity.class)
     ...
     * @param canEdit        是否可以编辑，false 则是家长身份
     * @param memberId       如果是家长身份，则memberId == curMemberId(UserHelper.getUserId)
     * @param schoolId       机构Id
     * @param courseVo       课程信息
     * @param isFromMyCourse 是否从我的学程中来
     */
    public static void start(Activity activity, String courseId, String chapterId, boolean canEdit,
                             String memberId, String schoolId, CourseVo courseVo,
                             CourseDetailParams courseDetailParams, boolean isFromMyCourse) {
        activity.startActivity(new Intent(activity, UnitExamListActivity.class)
                .putExtra("courseId", courseId)
                .putExtra("chapterId", chapterId)
                .putExtra("canEdit", canEdit)
    ...
    private void initViews() {
        ...
        topBar.setTitle(getResources().getString(R.string.unit_exam));

        testListAdapter = new UnitExamListAdapter(activity);
                ...
                if (result.getCode() == 0) {
                    ...
                    if (list != null) {
                        ...
                        for (int i = 0; i < tempList.size(); i++) {
                            if (tempList.get(i).getCexam().getType() == 1) {
                                list.add(tempList.get(i));
                            }
                        }
                        for (int i = 0; i < tempList.size(); i++) {
                            if (tempList.get(i).getCexam().getType() == 3) {
                                list.add(tempList.get(i));
                            }
                        }
                    }
