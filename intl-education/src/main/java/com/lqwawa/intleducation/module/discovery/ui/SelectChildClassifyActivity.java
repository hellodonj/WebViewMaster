...
public class SelectChildClassifyActivity extends MyBaseActivity {
    ...
    public static void startForResult(Activity activity, ClassifyVo vo,
        ...
        activity.startActivityForResult(new Intent(activity, SelectChildClassifyActivity.class)
    ...
    private void changeParam1Select(ClassifyVo vo) {
        ...
        if (topLevel.isEmpty()) {
            for (int i = 1; i < topAdapter.getCount(); i++) {
                ClassifyVo parentVo = (ClassifyVo) topAdapter.getItem(i);
                if (parentVo.getId() == vo.getParentId()) {
                    topAdapter.setSelectLevel(parentVo.getLevel());
                    changeTopSelect(parentVo);
                    break;
                }
            }
        }
            ...
            if (StringUtils.isValidString(topLevel)) {// 顶层不是全部
                ...
                for (int i = 0; i < pListTemp.size(); i++) {
                    if (pListTemp.get(i).getConfigType().equals(SORT_TYPE_SUBJECTS)
                            && pListTemp.get(i).getLabelId() == vo.getLabelId()) {
