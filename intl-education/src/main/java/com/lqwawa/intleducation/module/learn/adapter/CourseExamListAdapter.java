...
public class CourseExamListAdapter extends ExamListBaseAdapter{
    ...
    public CourseExamListAdapter(Activity activity) {
        super(activity);
        ...
        list = new ArrayList<ExamListVo>();img_width = (p_width  - DisplayUtil.dip2px(activity, 16)) / 7;
...
//                    }
            ...
            if (position == 0){
                showTitle = true;
            }else if(list.get(position - 1).getCexam().getType() != 4){
                showTitle = true;
            }
            if (showTitle){
                holder.titleLay.setVisibility(View.VISIBLE);
