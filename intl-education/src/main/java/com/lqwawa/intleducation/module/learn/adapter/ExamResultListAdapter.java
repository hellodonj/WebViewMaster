...
public class ExamResultListAdapter extends MyBaseAdapter {
    private Activity activity;
    private List<ExamItemVo> list;
    private LayoutInflater inflater;

    public ExamResultListAdapter(Activity activity) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView != null) {
            ...
            convertView = inflater.inflate(R.layout.mod_learn_exam_result_list_item, null);
            holder = new ViewHolder(convertView);
            convertView.setTag(holder);
        }
        holder.pager_index_tv.setText("" + (position + 1) + ".");
        holder.exam_title_tv.setText(vo.getCexer().getQuestion());
        if (vo.getCexer().getExerciseType() != 4) {
            ...
            if (vo.getUexer() == null || !vo.getCexer().getAnswer().equals(vo.getUexer().getAnswer())) {
                holder.result_iv.setImageDrawable(
                        activity.getResources().getDrawable(R.drawable.ic_rst_wrong));
            } else {
                holder.result_iv.setImageDrawable(
                        activity.getResources().getDrawable(R.drawable.ic_rst_right));
            }
        } else {
            ...
            if (vo.getUexer() == null){//答案为空
                ...
                holder.result_tv.setText("" +
                        activity.getResources().getString(R.string.result_score_with_colon)
