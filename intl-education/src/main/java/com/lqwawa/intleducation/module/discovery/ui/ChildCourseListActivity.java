...
public class ChildCourseListActivity extends MyBaseActivity implements View.OnClickListener {
    private static final String TAG = ChildCourseListActivity.class.getSimpleName();
    ...
    public static void start(Activity activity, ClassifyVo vo,
        ...
        activity.startActivity(new Intent(activity, ChildCourseListActivity.class)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (classifyVo.getLevel().equals("93")){
            SORT_TYPE_GLASS = "21";
            SORT_TYPE_SUBJECTS  = "22";
        }else{
            SORT_TYPE_GLASS = "11";
            SORT_TYPE_SUBJECTS  = "12";
        }
        ...
        if(filterHistoryVo == null && getIntent().getStringExtra("Sort") == null){
            saveFilterHistory();
            firstSelectClassify = true;
            SelectChildClassifyActivity.startForResult(activity, classifyVo,
    ...
    private void initList() {
        ...
        if (!topLevel.equals(classifyVo.getLevel()) && StringUtils.isValidString(topLevel)) {//不是全部
    ...
    private void sortCb1Click(View view) {
        ...
        SortLinePopupWindow linePopupWindow =
                new SortLinePopupWindow(ChildCourseListActivity.this, validClassifyList1, "" + param1LabelId,
    ...
    private void changeParam1Select(ClassifyVo vo) {
        ...
        if (topLevel.equals(classifyVo.getLevel())) {//是从全部进的
            List<ClassifyVo> pListTemp = new ArrayList<>();
            if (param1LabelId == CLS_ALL_VALUE) {//学段选全部
                for (int i = 0; i < classifyVo.getChildList().size(); i++) {
                    pListTemp.addAll(classifyVo.getChildList().get(i).getChildList());
                }
            } else {//学段没有选全部
                ...
                for (int i = 0; i < classifyVo.getChildList().size(); i++) {
                    if (classifyVo.getChildList().get(i).getId() == parentId) {
                        pListTemp.addAll(classifyVo.getChildList().get(i).getChildList());
                    }
                }
            }
    ...
    private void sortCb2Click(View view) {
        ...
        doNotCheckWithout(2);
        SortLinePopupWindow linePopupWindow =
                new SortLinePopupWindow(ChildCourseListActivity.this, validClassifyList2, "" + param2LabelId,
                        new SortLinePopupWindow.PopupWindowListener() {
                            @Override
                            public void onItemClickListener(int position, Object object) {
                                ClassifyVo vo = (ClassifyVo) object;
    ...
    private void changeParam2Select(ClassifyVo vo) {
        ...
        if (classifyVoParam2.getLevel().equals("")){
            classifyVoParam2 = classifyAllTheme;
        }
        if (param2LabelId != CLS_ALL_VALUE) {//不是全部
            if (!topLevel.equals(classifyVo.getLevel())) {// 顶层不是全部
                if (classifyVoParam2.getChildList() != null) {
                    classifyList3.addAll(classifyVoParam2.getChildList());
                }
            } else {//顶层是全部
                for (int i = 1; i < classifyList2.size(); i++) {
                    if (classifyList2.get(i).getConfigType().equals(SORT_TYPE_SUBJECTS)
                            && classifyList2.get(i).getLabelId() == vo.getLabelId()) {
                        classifyList3.addAll(classifyList2.get(i).getChildList());
                    }
                }
            }
    ...
    private void sortCb3Click(View view) {
        ...
        doNotCheckWithout(3);
        SortLinePopupWindow linePopupWindow =
                new SortLinePopupWindow(ChildCourseListActivity.this, validClassifyList3, "" + param3LabelId,
                        new SortLinePopupWindow.PopupWindowListener() {
                            @Override
                            public void onItemClickListener(int position, Object object) {
                                ClassifyVo vo = (ClassifyVo) object;
